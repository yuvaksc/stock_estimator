"""Preprocessing - flatten and clean the raw StockNet tweet dump.

Walks ``stock_data/stocknet-dataset-master/tweet/raw/<company>/<date>`` (one
JSON tweet per line, one file per day) and strips URLs, @mentions, #hashtags and
``$`` signs from each tweet's text, writing one tidy CSV per company.

Output  stock_data/processed_tweets/<company>.csv   (Date, Tweet)
Feeds   branches/branch21.py
"""
import os
import json
import re
...
def process_tweet_folder(root_folder, output_folder="stock_data/processed_tweets"):
...
# Run script
process_tweet_folder("stock_data/stocknet-dataset-master/tweet/raw")
